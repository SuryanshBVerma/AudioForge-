...
class NeuTTSAir:
    ...
    def _load_backbone(self, backbone_repo, backbone_device):
        """
        MODIFIED to load from a local path OR a Hugging Face repo.
        """
        print(f"Loading backbone from: {backbone_repo} on {backbone_device} ...")

        # GGUF loading remains the same
        if Path(backbone_repo).suffix == ".gguf" or "gguf" in str(backbone_repo).lower():
            # ... (keep the original GGUF loading code here) ...
            pass
        else:
            # Load from a local directory instead of downloading
